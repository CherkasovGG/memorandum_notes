...
@block_router.patch(
    ...
    responses={
        status.HTTP_200_OK: {
            "description": "Blocks data partially updated successfully."
        },
        status.HTTP_404_NOT_FOUND: {"description": "Block not found."},
    },
)
async def patch_transaction_route(
    data: dict[UUID, BlockPatchRequestDTO],
    x_user_id: Annotated[UUID, Header()],
    block_service: Annotated[BlockService, Depends(get_block_service())],
):
    """
    Partially update block transaction. The operation partially updates blocks with provided data.
    """

    data = await block_service.patch_transaction(
        x_user_id, {i: {k: v for k, v in data[i].model_dump().items() if v is not None} for i in data}
    )
